...
const Card = ({element, dataType }) => {
  ...
  return  (
    <Link to={`/${dataType}/details/${element.id}`} className=' w-[13vw] flex-shrink-0 '>
      ...
      <div className='flex justify-between w-full gap-2'>
        <p className='text-[#777777] font-semibold text-[1.2vw]'>{element.first_air_date || element.release_date || element.character || element.gender && (element.gender === 2 ? 'Male': 'Female') ||  'N/A'.split("-")[0]}</p>
        <p className='text-[#ffcb00] text-[1.2vw] font-semibold'><i className="ri-star-fill mr-[0.4vw] text-[1vw]"></i>{element.known_for_department || (element.vote_average && element.vote_average.toFixed(1))}</p>
      </div>
    </Link>
  )
}
